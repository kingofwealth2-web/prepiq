...
export default function SignUp() {
  ...
  const handleGoogle = async () => { await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/onboarding` } }) }
    ...
    <div style={{display:'flex',minHeight:'100vh',fontFamily:'var(--ff)',background:'var(--bg)',position:'relative',zIndex:1}}>
      <div style={{width:'460px',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',padding:'48px',background:'rgba(13,11,20,0.85)',backdropFilter:'blur(24px)',WebkitBackdropFilter:'blur(24px)',borderRight:'1px solid rgba(255,255,255,0.07)',position:'relative',zIndex:2}}>
        <div style={{width:'100%',maxWidth:'360px',animation:'fadeUp 0.4s ease both'}}>
          ...
          <div style={{fontSize:'0.65rem',fontWeight:'600',letterSpacing:'0.14em',textTransform:'uppercase',color:L,marginBottom:'6px'}}>Create account</div>
          <h2 style={{fontSize:'1.5rem',fontWeight:'800',color:'#fff',letterSpacing:'-0.03em',marginBottom:'6px'}}>Join PrepIQ today</h2>
          <p style={{fontSize:'0.86rem',color:'var(--ink-muted)',marginBottom:'24px'}}>Your WASSCE & BECE study companion.</p>
          ...
          <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'14px',marginBottom:'16px'}}>
            {[['full_name','Full name','text','e.g. Kwame Mensah'],['email','Email','email','you@gmail.com'],['password','Password','password','Min. 8 characters']].map(([name,lbl,type,ph]) => (
              <div key={name} style={{display:'flex',flexDirection:'column',gap:'5px'}}>
                <label style={{fontSize:'0.68rem',fontWeight:'600',color:'var(--ink-muted)',letterSpacing:'0.1em',textTransform:'uppercase'}}>{lbl}</label>
                <input name={name} type={type} placeholder={ph} value={form[name]} onChange={handleChange} required minLength={name==='password'?8:undefined} />
              </div>
            ))}
          ...
          <div style={{display:'flex',alignItems:'center',gap:'10px',margin:'4px 0 12px'}}>
            <div style={{flex:1,height:'1px',background:'rgba(255,255,255,0.08)'}} /><span style={{fontSize:'0.78rem',color:'var(--ink-faint)',flexShrink:0}}>or</span><div style={{flex:1,height:'1px',background:'rgba(255,255,255,0.08)'}} />
          </div>
          ...
          <p style={{textAlign:'center',fontSize:'0.82rem',color:'var(--ink-muted)',marginTop:'20px'}}>Already have an account? <Link to="/login" style={{color:L,fontWeight:'600',textDecoration:'none'}}>Log in</Link></p>
        </div>
      </div>
      <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'60px',position:'relative',zIndex:1}}>
        <div style={{maxWidth:'420px'}}>
          <div style={{fontSize:'3.4rem',fontWeight:'800',color:'#fff',lineHeight:1.05,marginBottom:'20px',letterSpacing:'-0.04em'}}>Your exam.<br /><em style={{color:L,fontStyle:'italic'}}>Your edge.</em></div>
          <p style={{fontSize:'1rem',color:'var(--ink-muted)',marginBottom:'36px',lineHeight:1.6}}>PrepIQ analyses years of WAEC patterns to tell you exactly what to study next.</p>
